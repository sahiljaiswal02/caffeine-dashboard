...
import { 
  TrendingUp, Wallet, Banknote, CalendarDays, 
  Package, LayoutDashboard, LogOut, Download,
  ChefHat, CheckCircle2, PlayCircle, Clock
} from "lucide-react";
...
export default function AdminDashboard() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "pending": return <span className="px-2.5 py-1 bg-slate-100 text-slate-800 rounded-md text-xs font-bold uppercase tracking-wider border border-slate-200">Pending</span>;
      case "preparing": return <span className="px-2.5 py-1 bg-orange-50 text-orange-800 rounded-md text-xs font-bold uppercase tracking-wider border border-orange-200">Preparing</span>;
      case "ready": return <span className="px-2.5 py-1 bg-blue-50 text-blue-800 rounded-md text-xs font-bold uppercase tracking-wider border border-blue-200">Ready</span>;
      case "delivered": return <span className="px-2.5 py-1 bg-emerald-50 text-emerald-800 rounded-md text-xs font-bold uppercase tracking-wider border border-emerald-200">Completed</span>;
      default: return <span className="px-2.5 py-1 bg-gray-100 text-gray-800 rounded-md text-xs font-bold uppercase tracking-wider border border-gray-200">{status}</span>;
    }
  };
  ...
  const renderOrderItems = (order: any) => (
    <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 mt-4">
      <h4 className="text-xs font-bold text-slate-800 uppercase tracking-wider mb-3">Order Contents</h4>
              ...
              <div>
                ...
                {(cartItem.selectedBread || (cartItem.selectedAddons && cartItem.selectedAddons.length > 0)) && (
                  <div className="mt-1 pl-2 border-l-2 border-orange-200 text-slate-500 text-xs space-y-1">
                    {cartItem.selectedBread && <div><span className="text-orange-500">•</span> {cartItem.selectedBread}</div>}
                    {cartItem.selectedAddons && cartItem.selectedAddons.length > 0 && (
                      <div><span className="text-orange-500">+</span> {cartItem.selectedAddons.map((a: any) => a.name).join(", ")}</div>
                    )}
                  </div>
                )}
    ...
    <div className="min-h-screen bg-slate-50 font-sans pb-12">
      ...
      <header className="bg-white border-b border-slate-200 px-4 sm:px-8 py-4 shadow-sm sticky top-0 z-30 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl sm:text-2xl font-black tracking-tight text-slate-900">Admin Dashboard</h1>
            <p className="text-sm text-slate-500 font-medium">Analytics & Business Overview</p>
          </div>
        </div>
        <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
          ...
          <button
            ...
            className="flex-1 sm:flex-none px-4 py-2 bg-orange-50 text-orange-700 border border-orange-200 rounded-lg text-sm font-bold hover:bg-orange-100 transition-colors shadow-sm flex items-center justify-center gap-2"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6">
          <div className="bg-white p-6 rounded-2xl shadow-sm hover:shadow-md transition-all duration-300 border border-slate-200 relative overflow-hidden">
            ...
            <div className="relative">
              ...
              <p className="mt-3 text-3xl sm:text-4xl font-black text-slate-900">{formatCurrency(dailyRevenue)}</p>
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm hover:shadow-md transition-all duration-300 border border-slate-200 relative overflow-hidden">
            ...
            <div className="relative">
              <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider flex items-center gap-2">
                <CalendarDays className="w-4 h-4 text-orange-500" /> Monthly Revenue
              </h3>
              <p className="mt-3 text-3xl sm:text-4xl font-black text-slate-900">{formatCurrency(monthlyRevenue)}</p>
              <div className="mt-4 flex items-center text-xs font-bold text-slate-600 bg-slate-100 w-fit px-3 py-1.5 rounded-lg border border-slate-200">
          ...
          <div className="bg-slate-900 p-6 rounded-2xl shadow-md hover:shadow-lg transition-all duration-300 border border-slate-800 relative overflow-hidden sm:col-span-2 md:col-span-1">
            ...
            <div className="relative z-10">
              ...
              <p className="mt-3 text-3xl sm:text-4xl font-black text-white">{formatCurrency(totalRevenue)}</p>
              ...
              <div className="flex justify-between items-center p-4 bg-slate-50 rounded-xl border border-slate-200">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-lg bg-orange-100 flex items-center justify-center">
                    <span className="font-black text-orange-700 text-[10px]">UPI</span>
                  </div>
                  <span className="font-bold text-slate-700 text-sm">Digital</span>
                </div>
                <span className="font-black text-slate-900 text-lg">{formatCurrency(upiRevenue)}</span>
              </div>
              <div className="flex justify-between items-center p-4 bg-slate-50 rounded-xl border border-slate-200">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-lg bg-emerald-100 flex items-center justify-center">
                    <span className="font-black text-emerald-700 text-[10px]">CASH</span>
                  </div>
                  <span className="font-bold text-slate-700 text-sm">Physical</span>
                </div>
                <span className="font-black text-slate-900 text-lg">{formatCurrency(cashRevenue)}</span>
              </div>
            </div>
          </div>
            ...
            <div className="bg-orange-50 p-6 rounded-2xl shadow-sm border border-orange-200 relative overflow-hidden flex flex-col justify-center">
              ...
              <div className="relative z-10">
                {topProductToday.count > 0 ? (
                  <>
                    <p className="text-2xl sm:text-3xl font-black text-orange-900 leading-tight">{topProductToday.name}</p>
                  ...
                  <p className="text-orange-800 font-medium bg-white px-4 py-2 rounded-lg w-fit border border-orange-100">No sales yet today</p>
                )}
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
              ...
              <div className="flex flex-col gap-3">
                <div className="flex items-center gap-4">
                  <div className="w-24 text-xs font-bold text-slate-500 uppercase flex items-center gap-1.5"><Clock className="w-3 h-3"/> Pending</div>
                  <div className="flex-1 h-3 bg-slate-100 rounded-full overflow-hidden">
                    <div className="h-full bg-slate-400 rounded-full" style={{ width: `${Math.min(100, kitchenStatus.pending * 10)}%` }}></div>
                  </div>
                  <div className="w-8 text-right font-black text-slate-700">{kitchenStatus.pending}</div>
                </div>
                <div className="flex items-center gap-4">
                  <div className="w-24 text-xs font-bold text-slate-500 uppercase flex items-center gap-1.5"><PlayCircle className="w-3 h-3"/> Prep</div>
                  <div className="flex-1 h-3 bg-slate-100 rounded-full overflow-hidden">
                    <div className="h-full bg-orange-400 rounded-full" style={{ width: `${Math.min(100, kitchenStatus.preparing * 10)}%` }}></div>
                  </div>
                  <div className="w-8 text-right font-black text-slate-700">{kitchenStatus.preparing}</div>
                </div>
                <div className="flex items-center gap-4">
                  <div className="w-24 text-xs font-bold text-slate-500 uppercase flex items-center gap-1.5"><CheckCircle2 className="w-3 h-3"/> Ready</div>
                  <div className="flex-1 h-3 bg-slate-100 rounded-full overflow-hidden">
                    <div className="h-full bg-blue-400 rounded-full" style={{ width: `${Math.min(100, kitchenStatus.ready * 10)}%` }}></div>
                  </div>
                  <div className="w-8 text-right font-black text-slate-700">{kitchenStatus.ready}</div>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden mb-8">
          <div className="px-4 sm:px-6 py-5 border-b border-slate-200 bg-white flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 w-full sm:w-auto">
              <span className="text-xs font-bold text-slate-500 bg-slate-100 px-3 py-1.5 rounded-lg border border-slate-200">Latest {Math.min(orders.length, 15)}</span>
              <button 
                onClick={generatePDF}
                ...
                {orders.slice(0, 15).map((order) => {
                  ...
                  return (
                    <React.Fragment key={order.id}>
                      ...
                      {isExpanded && (
                        <tr>
                          <td colSpan={6} className="bg-white px-6 py-4 border-b border-slate-200">
          ...
          <div className="md:hidden flex flex-col divide-y divide-slate-100">
            {orders.slice(0, 15).map((order) => {
              ...
              return (
                <div key={order.id} className="p-4 bg-white hover:bg-slate-50 transition-colors cursor-pointer" onClick={() => setExpandedOrderId(isExpanded ? null : order.id)}>
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      <span className="text-sm font-black text-slate-900">#{order.id.slice(-5).toUpperCase()}</span>
                      <div className="text-xs text-slate-500 font-medium mt-1">{formatTime(order.createdAt)}</div>
                    </div>
                    {getStatusBadge(order.status)}
                  </div>
                  <div className="flex justify-between items-end mt-3">
                    <div className="flex gap-2 items-center">
                      <span className="text-xs font-bold text-slate-600 bg-slate-100 px-2 py-1 rounded border border-slate-200 uppercase">{order.paymentMethod}</span>
                      <span className="text-xs font-medium text-slate-500">{itemCount} items</span>
                    </div>
                    <span className="font-black text-slate-900 text-lg">{formatCurrency(order.total || 0)}</span>
                  </div>
